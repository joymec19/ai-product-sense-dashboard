...
export function RunMarketButton({ sessionId, onComplete }: Props) {
  ...
  async function handleClick() {
    ...
    const channel = sb.channel(`session-market-${sessionId}`)
      .on('postgres_changes', {
    ...
    const res = await fetch(`/api/analysis/${sessionId}/market`, { method: 'POST' })
    if (!res.ok) {
