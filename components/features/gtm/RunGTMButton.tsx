...
export function RunGTMButton({ sessionId, onComplete }: Props) {
  ...
  async function handleClick() {
    ...
    const channel = sb.channel(`session-gtm-${sessionId}`)
      .on('postgres_changes', {
    ...
    const res = await fetch(`/api/analysis/${sessionId}/gtm`, { method: 'POST' })
    if (!res.ok) {
  ...
  return (
    ...
    >
      ...
      {running ? 'Running…' : 'Run GTM Strategy'}
    </button>
  )
}
